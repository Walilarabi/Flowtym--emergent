...
export const HotelProvider = ({ children }) => {
  ...
  const fetchHotels = useCallback(async () => {
    try {
      ...
      // Set current hotel from user's hotel_id or first hotel
  ...
  const updateRoom = async (roomId, roomData) => {
    if (!currentHotel) return;
    try {
      const response = await api.put(`/hotels/${currentHotel.id}/rooms/${roomId}`, roomData);
      setRooms((prev) => prev.map(r => r.id === roomId ? response.data : r));
      toast.success('Chambre mise a jour');
      return response.data;
    } catch (error) {
      toast.error('Erreur lors de la mise a jour de la chambre');
      throw error;
    }
  };
