...
export const ChannelView = () => {
  ...
  return (
    <div className="channel-saas-container animate-fade-in">
      ...
      {/* NAVIGATION TABS */}
      <div className="sub-nav scroll-hide" style={{ margin: '20px 0', padding: '6px', background: '#F1F5F9', border: '1px solid #E2E8F0', borderRadius: '12px', width: 'fit-content', display: 'flex', gap: '4px', maxWidth: '100%', overflowX: 'auto' }}>
        <button className={`module-tab ${activeSubTab === 'dashboard' ? 'active' : ''}`} style={{ color: activeSubTab === 'dashboard' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('dashboard')}><BarChart size={14} /> Dashboard</button>
        <button className={`module-tab ${activeSubTab === 'inventory' ? 'active' : ''}`} style={{ color: activeSubTab === 'inventory' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('inventory')}><Layout size={14} /> Inventaire Grid</button>
        <button className={`module-tab ${activeSubTab === 'rateshopper' ? 'active' : ''}`} style={{ color: activeSubTab === 'rateshopper' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('rateshopper')}><Activity size={14} /> Rate Shopper</button>
        <button className={`module-tab ${activeSubTab === 'reserves' ? 'active' : ''}`} style={{ color: activeSubTab === 'reserves' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('reserves')}><Inbox size={14} /> Réservations</button>
        <button className={`module-tab ${activeSubTab === 'bookingengine' ? 'active' : ''}`} style={{ color: activeSubTab === 'bookingengine' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('bookingengine')}><Globe size={14} /> Booking Engine</button>
        <button className={`module-tab ${activeSubTab === 'mapping' ? 'active' : ''}`} style={{ color: activeSubTab === 'mapping' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('mapping')}><Layers size={14} /> Mapping OTA</button>
        <button className={`module-tab ${activeSubTab === 'rms' ? 'active' : ''}`} style={{ color: activeSubTab === 'rms' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('rms')}><Cpu size={14} /> RMS Intelligence</button>
        <button className={`module-tab ${activeSubTab === 'config' ? 'active' : ''}`} style={{ color: activeSubTab === 'config' ? '#4361EE' : '#64748B' }} onClick={() => setActiveSubTab('config')}><Settings size={14} /> Configuration</button>
      </div>
